...
namespace Gravix 
{
	...
	void SceneSerializer::SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		for (auto typeIndex : ComponentRegistry::Get().GetComponentOrder())
		{
			...
			if (info.SerializeFunc && info.GetComponentFunc)
			{
				...
				{
					// Serialize all instances of this component
				...
				{
					// Single instance component - serialize normally
	...
	bool SceneSerializer::Deserialize(const std::filesystem::path& filepath, YAML::Node* outNode)
	{
		std::ifstream stream(filepath);
		std::stringstream strStream;
