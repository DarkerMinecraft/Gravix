...
# Run setup script (configures CMake, generates Visual Studio solution)
Scripts\Setup.bat
```

### Development Builds
```bash
# Build Debug configuration (from Visual Studio solution generated by setup)
...
```bash
# Build Release and create distribution package
...
- Collects executables, assets, and DLLs from `build/Release/Orbit/`
- Creates timestamped ZIP in `dist/` directory

**Build Output**: Orbit editor executable is in `build/[Debug|Release]/Orbit/`

### .NET Scripting
```bash
# Build C# scripting assembly
cd Gravix-ScriptCore
dotnet build
```

**Prerequisites**: Windows 10/11, Visual Studio 2019+ with C++ support, CMake 3.20+, Vulkan SDK, Python 3.7+, .NET 9.0 SDK
